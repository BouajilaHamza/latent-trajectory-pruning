...
def load_math500_subset(split: str = "test", num_samples: int = 500) -> list[dict]:
    ...
    dataset = dataset.select(range(min(num_samples, len(dataset))))
    return [
        {"question": item["problem"], "answer": item["solution"]} for item in dataset
    ]
...
def load_gpqa_subset(split: str = "train", num_samples: int = 100) -> list[dict]:
    ...
    for item in dataset:
        ...
        formatted_data.append({"question": question_text, "answer": correct_letter})
    ...
    if (
        ...
        return tokenizer.apply_chat_template(
            [
                {
                    "role": "system",
                    "content": "You are a helpful expert answering multiple choice questions.",
                },
    ...
    if hasattr(tokenizer, "apply_chat_template") and getattr(
        ...
        return tokenizer.apply_chat_template(
            [
                {
                    "role": "system",
                    "content": "You are a helpful mathematical reasoning assistant.",
                },
                {"role": "user", "content": prompt},
            ],
            tokenize=False,
